...
export function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      const { data } = await apiClient.post<AuthResult>('/api/auth/login', { email, password });
      login(data);
      navigate('/');
    } catch {
          ...
          <div className="relative h-full flex flex-col justify-end p-16 z-10">
            <div className="mb-8">
              <span className="text-on-primary font-headline text-5xl font-extrabold tracking-tighter leading-tight">
                The new way<br />to borrow.
              </span>
              <p className="text-on-primary-container mt-6 text-xl max-w-md font-medium leading-relaxed opacity-90">
                Join a curated community where high-end gear and everyday essentials find new life through trusted exchange.
              </p>
            </div>
          ...
          <div className="flex-1 flex flex-col justify-center px-8 sm:px-12 lg:px-24 py-12">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center ml-1">
                  <label htmlFor="password" className="block text-sm font-bold text-on-surface-variant font-label">
                    Password
                  </label>
                  <span className="text-xs font-bold text-primary cursor-default">Forgot?</span>
                </div>
            ...
            <div className="mt-10 text-center">
              <p className="text-on-surface-variant font-medium">
                ...
                <Link to="/register" className="text-tertiary font-extrabold hover:underline ml-1">
                  Sign Up
                </Link>
              </p>
            </div>
